...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased text-gray-900 bg-gray-100">
            ...
            <div x-show="isOpen" 
                ...
                <div class="bg-gradient-to-r from-sky-500 to-blue-600 p-4 flex items-center justify-between shadow-md">
                    <div class="flex items-center space-x-3">
                        ...
                        <div class="text-white">
                            <h3 class="font-bold text-sm tracking-wide">CORP Assistant</h3>
                ...
                <div class="flex-1 p-4 overflow-y-auto bg-slate-50 space-y-6" id="chat-box">
                    <div class="flex items-start gap-2.5">
                        ...
                        <div class="flex flex-col gap-1">
                            <span class="text-[10px] text-gray-500 font-semibold ml-1">CORP Assistant</span>
                            <div class="bg-white p-3.5 rounded-2xl rounded-tl-none shadow-sm border border-gray-100 text-sm text-gray-700 leading-relaxed max-w-[260px]">
                                Halo, <b>{{ Auth::user()->name }}</b>! 👋 <br>Saya CORP Assistant. Ada yang bisa dibantu?
                            </div>
                        </div>
                    </div>
                ...
                <div class="p-4 bg-white border-t border-gray-100">
                    ...
                    <div class="text-center mt-2">
                         <span class="text-[10px] text-gray-300 font-medium tracking-wide">Powered by RAHMAT AI</span>
                    </div>
                </div>
            </div>
        ...
        @if (session('toast_success') || session('success'))
            <div x-data x-init="Toastify({ text: '{{ session('toast_success') ?: session('success') }}', duration: 3000, gravity: 'top', position: 'right', style: { background: 'linear-gradient(to right, #10b981, #059669)', borderRadius: '0.5rem' } }).showToast();"></div>
        @endif
        @if (session('toast_danger') || session('error'))
            <div x-data x-init="Toastify({ text: '{{ session('toast_danger') ?: session('error') }}', duration: 3000, gravity: 'top', position: 'right', style: { background: 'linear-gradient(to right, #ef4444, #dc2626)', borderRadius: '0.5rem' } }).showToast();"></div>
        @endif
        @if ($errors->any())
